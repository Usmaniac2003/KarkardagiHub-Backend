...
exports.getAllNotifications = async (req, res) => {
  try {
    const { type, generated_by, limit = 10, page = 1 } = req.query;  // Query params for filtering & pagination
    const query = {};

    if (type) query.type = type; // Filter by notification type
    if (generated_by) query.generated_by = generated_by; // Filter by admin who generated the notification
    ...
    const notifications = await Notification.find(query)
      ...
      .sort({ created_at: -1 }); // Sort by created_at (newest first)
...
exports.addNotifications = async (req, res) => {
  try {
    const { title, description, generated_by, type } = req.body;  // Expected notification fields from request body

    if (!title || !description ) {
      return res.status(400).json({ message: 'Title, description, and generated_by are required' });
    }

    const newNotification = new Notification({
      title,
      description,  // Reference to the admin user who created this notification
      type: type || 'Deadline',  // Default to 'Deadline' if no type provided
    });
